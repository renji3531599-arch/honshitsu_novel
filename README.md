...
| BGM | 手続き生成 24曲＋SE24種 | 手続き生成 24曲＋SE16種 |
...
`assets/` には**背景25枚と立ち絵78枚の実画像**が入っている。CG7枚（名場面5＋ED2）は**名前だけ台帳に登録**してあり、
...
* 本編で出さないCGは2026-09-11に11枚降板（`reserve` 化）→ **2026-09-12 に台帳・実ファイルとも削除**。
  さらに **2026-09-15 に6枚を追加降板**（直後のCGと2連発になる準備絵・立ち絵で足りる表情アップ・
  隣接シーンとの重複構図）。さらに19枚を撤去し、CGは物語の核となる
  **7枚（名場面5＋ED2）**（基準・経緯は `docs/CG_GUIDE.md` 冒頭と `docs/ASSET_MANIFEST.md`「2026-09-15 CG追加降板」）。
* 素材IDは脚本内で `bg_hokutou_kyoshitsu_asa`（=ファイル名stem）でも `BG01` でも書ける。
* **1素材ずつの解説**（どのシーンで何回出るか／未使用差分／Ken Burns 有無／降板理由）は自動生成：
  `assets/README.md`（総）／ `assets/bg|cg|chr/README.md` ／ **`docs/CG_GUIDE.md`（CG全7枚を物語順に）**。
  生成用のコピーペースト可能な英語プロンプトは **`docs/CG_PROMPTS.md`**（**1枚ごとに「添付する背景・立ち絵の実ファイル」指定つき**）にまとめてある。
  台帳か脚本を直したら `node tools/gen_asset_md.mjs` で再生成。
* 差し替え後は **`sw.js` の `CACHE`（現在 `honshitsu-v11`）を必ず上げる**。CacheFirst で画像を返すため、
...
js/audio.js                   … BGM24種＋SE24種の手続き生成（WebAudio）
...
CONFIGに4つつまみを追加：背景の切り替え／立ち絵の動き／画像合成／テキスト窓をぼかす。
...
* 背景25枚・立ち絵78枚は実画像を収録済み。CGは7枚（名場面5＋ED2）が台帳スロットのみで実ファイル未配置 ――
  構図・拡大表示（`cg_chizutsutsu_kobore_shashin` と `cg_omoide_chikeizu_kansei`）分の作り込みはエンジン側に入れてある。
* 心Point は非表示が作法なので、HUD には出さない（エンドカードで初めて明かされる）。
* 脚本の行数配分は喜劇8:感情2。感情ピークの直前直後に必ずツッコミを1発置く（企画書 14章のメモ）。


---

# 版B: `game/` 以下のビルド（PR #2 で main に統合済み）
...
- **エンジン**: セーブ12スロット＋クイック＋オートセーブ／黒字リプレイ方式ロード／バックログ／既読スキップ（Ctrl）／オートモード／ギャラリー7枚（名場面5＋ED2）／**✝本質✝辞典**（用語自動収集）／ENDリスト／設定
- **2026-09-12**: UI画像20枚を撤去（CSS/SVG描画で全代替）、CGは「印象的で感動的なシーンのみ」の32枚に整理、白紙プレースホルダ256枚を削除（背景24枚だけが実画像）。経緯は `docs/UI_CG_2026-09-12.md`
- **2026-09-15**: 効果が薄く構図が重複するCG 6枚を追加降板し**26枚**に（版Aと同時。脚本・マニフェストから削除、ギャラリーの分母も自動で縮小）。記録は `docs/ASSET_MANIFEST.md`「2026-09-15 CG追加降板」
...
両版とも **26→7枚（約73%削減）**。各ルート・各ENDへの一律配置を廃止。
全14ENDの物語は残し、CGを外した場面は背景＋立ち絵で進めます。
制作対象と選定理由: `docs/CG_PRUNING_2026-09-15.md`。
