...
`assets/` には**背景25枚と立ち絵78枚の実画像**が入っている。CG26枚（名場面12＋ED14）は**名前だけ台帳に登録**してあり、
...
* 本編で出さないCGは2026-09-11に11枚降板（`reserve` 化）→ **2026-09-12 に台帳・実ファイルとも削除**。
  ...
  隣接シーンとの重複構図）。これでCGは「印象的で感動的なシーンにだけ置く」基準の
  **26枚（名場面12＋ED14）**（基準・経緯は `docs/CG_GUIDE.md` 冒頭と `docs/ASSET_MANIFEST.md`「2026-09-15 CG追加降板」）。
* 素材IDは脚本内で `bg_hokutou_kyoshitsu_asa`（=ファイル名stem）でも `BG01` でも書ける。
* **1素材ずつの解説**（どのシーンで何回出るか／未使用差分／Ken Burns 有無／降板理由）は自動生成：
  `assets/README.md`（総）／ `assets/bg|cg|chr/README.md` ／ **`docs/CG_GUIDE.md`（CG全26枚を物語順に）**。
  生成用のコピーペースト可能な英語プロンプトは **`docs/CG_PROMPTS.md`**（**1枚ごとに「添付する背景・立ち絵の実ファイル」指定つき**）にまとめてある。
  台帳か脚本を直したら `node tools/gen_asset_md.mjs` で再生成。
* 差し替え後は **`sw.js` の `CACHE`（現在 `honshitsu-v10`）を必ず上げる**。CacheFirst で画像を返すため、
...
* 背景25枚・立ち絵78枚は実画像を収録済み。CGは26枚（名場面12＋ED14）が台帳スロットのみで実ファイル未配置 ――
...
- **エンジン**: セーブ12スロット＋クイック＋オートセーブ／黒字リプレイ方式ロード／バックログ／既読スキップ（Ctrl）／オートモード／ギャラリー26枚（名場面12＋ED14）／**✝本質✝辞典**（用語自動収集）／ENDリスト／設定
