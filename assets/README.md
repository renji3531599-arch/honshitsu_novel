...
- `cg/README.md` ― CG 7 枚（出番・Ken Burns・差し替え仕様）
...
| `cg/` | 7 | 実画像 0・未配置 7（白紙削除済み） | 1920×1080（16:9）／cover・kb で最大1.11倍 | ―（暗色ベタのみ） |
...
3. `sw.js` の `CACHE`（現在 `honshitsu-v11`）を上げる ← **忘れると古いキャッシュを返す**（`--bump` で自動）
