...
**背景（`assets/bg/` 25枚）と立ち絵（78/78枚）が実画像**。CGは**台帳にスロット名だけ**登録してあり、
...
- `cg/README.md` ― CG 26 枚（出番・Ken Burns・差し替え仕様）
...
| `cg/` | 26 | 実画像 0・未配置 26（白紙削除済み） | 1920×1080（16:9）／cover・kb で最大1.11倍 | ―（暗色ベタのみ） |
| `chr/` | 78 | 実画像 78・未配置 0（白紙削除済み） | 840×1280（21:32）／透過・下揃え | シルエット（`figureSVG`） |
...
3. `sw.js` の `CACHE`（現在 `honshitsu-v10`）を上げる ← **忘れると古いキャッシュを返す**（`--bump` で自動）
